...
4. Roll: `int attributeValue = Character.GetEffectiveAttribute(selectedAttribute); int diceRoll = GameMechanics.Dice.Roll4dFPlus(); int effectiveValue = attributeValue + boostValue; int totalRoll = effectiveValue + diceRoll; int sv = totalRoll - targetValue;`
   - IMPORTANT: Anonymous actions use RAW attribute value, NOT the AS formula (no skill level, no -5).
5. Count wounds: `int woundCount = Character.Effects.Count(e => e.EffectType == EffectType.Wound);` Apply wound penalty: `effectiveValue -= woundCount * 2;` (apply BEFORE calculating totalRoll).
6. Check multi-action penalty: `if (Character.ActionPoints.ActionsTakenThisRound > 1) effectiveValue -= (Character.ActionPoints.ActionsTakenThisRound - 1) * 2;` (apply BEFORE calculating totalRoll, AFTER incrementing ActionsTakenThisRound).
7. Set `hasRolled = true`, store result fields.
8. Save: `if (Character is Csla.Core.ISavable savable) await savable.SaveAsync();`
