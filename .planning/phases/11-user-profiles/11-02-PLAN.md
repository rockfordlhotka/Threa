...
must_haves:
  truths:
    ...
    - "User can view their own profile page at /profile/{id}"
    - "Users can view other users' public profiles at /profile/{id}"
  ...
  artifacts:
    ...
    - path: "Threa/Threa.Client/Components/Pages/Profile.razor"
      provides: "Public profile viewing page"
      contains: "@page \"/profile/{Id:int}\""
  ...
  key_links:
    - from: "Threa/Threa.Client/Components/Pages/Player/PlayerEdit.razor"
      ...
      pattern: "vm.Model"
    - from: "Threa/Threa.Client/Components/Pages/Profile.razor"
      to: "GameMechanics/Player/ProfileInfo.cs"
      via: "IDataPortal<ProfileInfo> fetch"
      pattern: "profilePortal.FetchAsync"
...
<objective>
Create the UserAvatar component, Profile.razor page for viewing profiles, and enhance the profile editing UI with Gravatar preview, then integrate avatars throughout the application (nav bar, admin user list). Users can edit their profile and view any user's public profile.

Purpose: Delivers the user-facing profile management experience. Users can set display names, provide email for Gravatar, see their avatar throughout the UI, and view other users' public profiles.

Output: Reusable UserAvatar component, Profile.razor page, enhanced profile edit page with live Gravatar preview, avatars in nav and admin UI.
</objective>
...
@.planning/phases/11-user-profiles/11-01-SUMMARY.md
...
- GameMechanics/Player/ProfileInfo.cs - Read-only profile BO from 11-01
</context>
...
<task type="auto">
  <name>Task 3: Create Profile.razor for viewing public profiles</name>
  <files>
    - Threa/Threa.Client/Components/Pages/Profile.razor
  </files>
  <action>
Create Profile.razor page for viewing any user's public profile.

Uses ProfileInfo read-only business object created in 11-01.
Route: /profile/{id} where id is the player ID.
...
Note: This page shows only public info (display name, avatar). Email is used for Gravatar lookup but not displayed. ContactEmail privacy is handled by the avatar component (hashes the email for Gravatar URL).
  </action>
  <verify>
    - Build succeeds: `dotnet build Threa/Threa.Client/Threa.Client.csproj`
    - File exists: `ls Threa/Threa.Client/Components/Pages/Profile.razor`
    - Page has @page "/profile/{Id:int}" directive
  </verify>
  <done>
    - Profile.razor created at /profile/{id} route
    - Displays user's avatar and display name
    - Uses ProfileInfo business object for data
    - Handles loading states and errors gracefully
  </done>
</task>

<task type="auto">
  <name>Task 4: Add avatar to navigation bar and admin user list</name>
  <files>
    - Threa/Threa/Components/Layout/NavMenu.razor
    - Threa/Threa.Client/Components/Pages/Admin/Users.razor
  </files>
...
4. Verify profile page shows:
   ...
   - Use Gravatar toggle
   - Live avatar preview
5. User can view their own profile at /profile/{their-id}
6. User can view other users' public profiles at /profile/{other-id}
7. Save changes and verify they persist
8. Verify nav shows logged-in user
9. Verify admin user list shows avatar column (as admin user)
</verification>
...
- Profile.razor page shows public profile for any user at /profile/{id}
