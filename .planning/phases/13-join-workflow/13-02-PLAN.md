...
must_haves:
  truths:
    ...
    - "JoinRequestProcessor publishes JoinRequestMessage on approval and denial for player notification"
  ...
  key_links:
    ...
    - from: "JoinRequestProcessor"
      to: "ITimeEventPublisher.PublishJoinRequestAsync"
      via: "notification on approval/denial"
      pattern: "PublishJoinRequestAsync"
---
...
Add required usings: Csla, System, System.Threading.Tasks, Threa.Dal, Threa.Dal.Dto, GameMechanics.Messaging
  </action>
  <verify>
`dotnet build GameMechanics/GameMechanics.csproj` succeeds
  </verify>
  <done>
JoinRequestProcessor validates GM ownership, handles approval (attaches character to table, updates status, publishes approval notification) and denial (publishes denial notification, deletes request). Returns character name for UI message.
  </done>
</task>
...
- Approval atomically updates request status AND attaches character to table AND publishes notification
- Denial publishes notification THEN deletes request (order matters - notify before delete)
- Notification messages include character name, table name, and status
