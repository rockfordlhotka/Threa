...
<task type="auto">
  <name>Task 2: Add Capacity and Type Restriction Warnings (Non-Blocking)</name>
  <files>Threa/Threa.Client/Components/Pages/GamePlay/TabPlayInventory.razor</files>
  <action>
Add warning messages for capacity and type restrictions (these warn but allow placement per CONTEXT.md):
...
4. Add type restriction validation method (warning only, per CONTEXT.md):
```csharp
...
5. Add capacity warning check (warning only, per CONTEXT.md):
```csharp
...
6. Update MoveToContainer to include capacity and type warnings (these are non-blocking):
```csharp
...
```
  ...
  <verify>dotnet build Threa/Threa.sln --no-restore</verify>
  <done>Capacity warnings display when placing items would exceed weight/volume (but placement succeeds). Type restriction warnings display when placing wrong item type (but placement succeeds). Both are non-blocking per CONTEXT.md "warn but allow" design.</done>
</task>

<task type="auto">
  <name>Task 3: Add Nesting Enforcement (Blocking) and Drop Container Confirmation</name>
  <files>Threa/Threa.Client/Components/Pages/GamePlay/TabPlayInventory.razor</files>
  <action>
Add nesting validation (blocking) and container drop confirmation dialog:

1. Add nesting validation method (this BLOCKS, unlike capacity/type warnings):
```csharp
...
2. Update MoveToContainer to check nesting FIRST (blocking validation):
Add this check at the beginning of MoveToContainer, before type/capacity warnings:
```csharp
// In MoveToContainer, add after item/container null checks:
...
// ... rest of the method (type/capacity warnings, actual move)
```

3. Update ConfirmDropItem to handle containers with contents:
```csharp
...
4. Add ConfirmDropContainerWithContents method with three-option dialog:
```csharp
...
```
  ...
  <verify>dotnet build Threa/Threa.sln --no-restore</verify>
  <done>Nesting is blocked with clear error message: (1) cannot place items in containers that are already nested inside other containers, (2) cannot place non-empty containers into other containers, (3) empty containers CAN be nested (one level per CONTEXT.md). Container drop shows three-option dialog (Cancel/Empty First/Drop All).</done>
</task>
...
- Capacity warnings when exceeding limits (but allows placement)
- Type restriction warnings (but allows placement)
- Nesting enforcement that BLOCKS: (a) items into nested containers, (b) non-empty containers into containers
- Nesting that ALLOWS: empty containers can be nested (one level only)
...
5. Test capacity warning (non-blocking):
...
6. Test nesting enforcement (blocking):
   a. Put a container inside another container (should work if empty)
   b. Try to move an item into that nested container
      - Should see ERROR message and item should NOT move
   c. Try to move a non-empty container into another container
      - Should see ERROR message and container should NOT move
   d. Empty container CAN go into another container (should work)

7. Test type restriction warning (non-blocking):
...
- Capacity warnings appear but DON'T block operations (per CONTEXT.md)
- Type restriction warnings appear but DON'T block operations (per CONTEXT.md)
- Nesting IS blocked with clear error message (per CONTEXT.md one-level rule)
- Empty containers CAN be nested (one level only)
...
<success_criteria>
- INV-16: Container weight limits enforced via warnings (but allows placement per CONTEXT.md)
- INV-17: Container volume limits enforced via warnings (but allows placement per CONTEXT.md)
- INV-18: Container type restrictions enforced via warnings only (per CONTEXT.md)
</success_criteria>
