...
must_haves:
  ...
  key_links:
    ...
    - from: "TabPlayInventory.razor"
      to: "inventoryItems collection"
      via: "Client-side filtering for container contents (performance optimization - all items pre-loaded)"
      pattern: "inventoryItems\\.Where.*ContainerItemId"
---
...
3. Add method to get container contents using client-side filtering (items already loaded for performance):
```csharp
