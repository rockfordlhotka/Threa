...
<task type="auto">
  ...
  <action>
    ...
    **Part B: Replace existing NPC panel in GmTable.razor (DEFENSIVE)**

    1. **Search for the existing NPCs panel** (around line 188-210):
    ...
    3. **DEFENSIVE CLEANUP - Only remove if found:**
       Search for and remove the following items IF they exist. Skip any that don't exist:

       - `AddNpc` method (if exists: `private void AddNpc()`)
       - `RemoveNpc` method (if exists: `private void RemoveNpc(Guid npcId)`)
       - `npcs` collection field (if exists: `private List<NpcInfo>? npcs;`)
       - `NpcInfo` record type (if exists: `private record NpcInfo(...)`)

       **IMPORTANT:** Before removing each item:
       - Search the file for other usages of that identifier
       - If the item is used elsewhere (not just the NPC panel), do NOT remove it
       - If the item doesn't exist, skip that removal step

       This defensive approach prevents build errors if the codebase structure has changed.
