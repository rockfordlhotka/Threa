...
<task type="auto">
  ...
  <action>
    ...
    **Part B: Update CharacterDetailModal.razor to use components**
    ...
    3. In OnCharacterChanged, load GM notes from tableCharacters collection:
       ```csharp
       private async Task OnCharacterChanged()
       {
           ...
           // Get GM notes from table character record
           var tableChar = AllCharacters.FirstOrDefault(tc => tc.CharacterId == selectedCharacterId);
           // Note: TableCharacterInfo may need GmNotes property added, or load separately via tableDal
           currentGmNotes = null; // Placeholder - will be populated from data
    ...
    4. If TableCharacterInfo doesn't have GmNotes, add it to the info object or fetch separately from tableDal in the modal's OnInitializedAsync.
  </action>
  <verify>
    - Build succeeds: `dotnet build Threa.sln`
  ...
  <done>
    All three tab components created and wired into CharacterDetailModal. GM notes persist on blur.
  </done>
</task>
...
6. Switching characters via dropdown refreshes all tab content
