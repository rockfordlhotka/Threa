...
must_haves:
  truths:
    ...
    - "GM sees overflow warning when VIT damage would exceed pool and result in wounds"
    - "GM must confirm overflow before FAT damage cascades to VIT"
    - "GM must confirm overheal before healing exceeds pool maximum"
    - "GM sees overheal warning when healing would exceed pool maximum (inline alert, separate from Plan 01's PendingPoolBar badge)"
    - "GM can confirm or cancel after seeing warning"
    - "GM can view current and pending FAT/VIT values before applying changes (via existing PendingPoolBar display)"
       ...
       if (healthMode == HealthMode.Damage)
       {
           ...
           else if (pool == "VIT")
           {
               ...
               if (projectedVit < 0)
               {
                   ...
                   overflowWarningMessage = $"This will exceed VIT capacity. {excess} points of damage beyond VIT will result in wounds.";
                   showOverflowWarning = true;
                   return;
               }
           }
...
4. Verify GM can view current and pending FAT/VIT values (HLTH-06) - the PendingPoolBar components display these values before any changes are applied
5. Verify single health management card with mode toggle
6. Test damage mode:
   - Apply small FAT damage (no warning)
   - Apply large FAT damage that overflows (warning appears in inline alert)
   - Confirm overflow warning
7. Test healing mode:
   - Apply small healing (no warning)
   - Apply large healing that overheals (warning appears in inline alert)
   - Cancel overheal warning
8. Verify real-time updates propagate to dashboard
9. Test both themes for visual consistency

Note: Plan 02's overheal warning is an inline alert in CharacterDetailGmActions, separate from Plan 01's overheal badge in PendingPoolBar. These can execute in parallel (Wave 1).
</verification>
...
- Overheal warning appears when healing exceeds maximum (inline alert in this component)
- Confirm/Cancel buttons on warnings work correctly
- GM can view current/pending pool values before applying changes (HLTH-06 - existing PendingPoolBar display)
- Real-time updates continue to function
- Visual styling matches theme (fantasy/scifi)
- All 7 HLTH requirements satisfied (HLTH-01 through HLTH-07)
</success_criteria>
