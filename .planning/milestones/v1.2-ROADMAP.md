# Milestone v1.2: GM Table & Campaign Management
...
Enable Game Masters to create and manage campaign tables with player character joining, real-time status monitoring, and comprehensive time control.

**Milestone Goal:** Players and Game Masters can collaborate in organized campaign tables with real-time character tracking and flexible time management for both combat and exploration scenarios.
...
**Success Criteria:**
1. GM can advance time by round (3 seconds), minute, turn (10 minutes), hour, day, or week
...
- Epoch-based world time tracking for flexible campaign time management
- Context-aware time buttons: show only relevant controls for current mode (combat vs exploration)
- "In Rounds: Round X" combined badge instead of separate Round/Combat indicators
- Time skip processing capped at 100 iterations to prevent UI freeze on large calendar skips
- Five-tab modal for character details prioritizing GM workflow (Actions, Sheet, Inventory, Grant Items, Narrative)
- Two-button FAT/VIT layout eliminates dropdown step for faster damage/healing application
- RadzenDialog must be placed in interactive context for DialogService to work
- Character-specific GM notes stored per table-character pair for campaign context
- Real-time updates via CharacterUpdateMessage reuse existing infrastructure
- ITimeEventSubscriber subscription pattern for consistent time-based updates
- 500ms delay before character list refresh to debounce rapid time events
- JoinRequest with status enum (Pending/Approved/Denied) for clean state management
- NPC placeholder in v1.2 lays foundation, full NPC management deferred to v1.3+

**Issues Resolved:**

- Pre-existing bug: Play.razor now subscribes to TimeSkipReceived for non-combat time advancement
- Modal rendering fixed by placing RadzenDialog in interactive context
- GM notes persistence fixed by loading directly from DAL instead of stale AllCharacters collection
- Click-outside-to-close enabled for touch device accessibility
- Removed redundant center panel for cleaner 2-column dashboard layout

**Technical Debt Incurred:**

- Known technical debt from v1.0 remains (non-blocking):
...
- GM character manipulation (apply damage/healing, add/remove effects, edit stats)
- Full NPC management system (create, edit, add to table, status tracking)
- Effect management UI (edit properties, duration, remove effects)
- AP Max recalculation bug fix (should recalculate when skills change, not just on creation)
- FAT recovery warning when VIT is low (VIT 1-4 affects FAT recovery rate)

---

_For current project status, see .planning/ROADMAP.md (archived - create fresh roadmap for next milestone)_
